...
/**
 * Count the number of connected components in an evolving boids simulations
 * where boids are nodes and neighbors relations defines edges.
 */
public class BoidsCC {
	public static void main(String args[]) {
		...
		try {
			...
			graph.getSpecies("moustik").setReproductionProbability(
					new Probability.ConstantProbability(0));
			graph.getSpecies("moustik").setDeathCondition(
					new Probability.ConstantProbability(0));
		} catch (IOException e) {
			e.printStackTrace();
		}
		...
		while (steps < 10000) {
			graph.step();
			scc.setAttribute("ui.label",
					String.format("%d", cc.getConnectedComponentsCount()));
			steps++;
			nap(20);
		}
	}

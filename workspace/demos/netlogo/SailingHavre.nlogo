...
to setup
  ...
  create-boats population
    [  
      ...
      setxy max-pxcor / 2 -(random 30) (10 + random 20)
      set start? false
      set arrival? false
      set color 7 + 10 * random 14  
     ]
  place-buoys
  set timeBeforeStart random MaxTimeBeforeStart  
  set ranking [] set rankRed [] set rankGreen []
  reset-ticks
end

to place-buoys
  set xdepart max-pxcor - 40
...
to go-where-collision-avoidance [ x y ]
  let where 0
  facexy x y                                           
  set heading heading + random-float rate-of-random-turn  / 2 - random-float rate-of-random-turn
  ifelse (any? other boats in-cone boat-radius-length boat-radius-angle)
      [ rt 2 fd 0.01 ]             
      [
          set where random-float boat-speed + 0.2
          fd where       ] 
...
to go
  ...
  ifelse ticks < TimeBeforeStart - 20 
  [
    ask boats [go-where-collision-avoidance xdepart - (10 + random 40)  (random max-pycor)]
  ]
  [
    ifelse ticks > TimeBeforeStart 
    [ 
